#!/bin/bash
#SBATCH --job-name=SSRC
#SBATCH --time=01:00:00
#SBATCH --cpus-per-task=12
#SBATCH --account=CORE-WCHN-MELD-SL2-CPU
#SBATCH --output=logs/SSr_resize_crop_%A_%a.txt
#SBATCH --array=2-73
...
OUTDIR=${rdspath}/growthcharts/dev/BOPS/code/BCP-pipeline-testing/BIDS/derivatives/synthseg-robust-resize-crop/${subject}/${session}/
...
singularity run --cleanenv \
  ...
  ${FS_SINGULARITY} \
  mri_synthstrip \
  -i /derivatives/${resampled_scan}/${resampled_scan}.nii.gz \
  -o /derivatives/${resampled_scan}/${skullstripped_scan}.nii.gz \
  -m /derivatives/${resampled_scan}/${brainmask}.nii.gz
