...
# Input paths
allt2s=${rdspath}/growthcharts/dev/${study}/code/BCP-pipeline-testing/all-T2s.tsv
...
line=$(sed -n "$((SLURM_ARRAY_TASK_ID))p" "$allt2s")
...
derivatives_dir=${bidsdir}/${subject}/${session}/${subject}_${session}_acq-MRP_run-001_${res}_T2w/
synthseg_segmentation=${derivatives_dir}/${subject}_${session}_acq-MRP_run-001_${res}_desc-cropped_T2w_seg.nii.gz
...
cp ${input_dir}/${subject}_${session}_acq-MRP_run-001_T2w.nii.gz ${derivatives_dir}/

seg_resize_space=${subject}_${session}_acq-MRP_run-001_${res}_desc-resized-space_T2w_seg.nii.gz
seg_synthseg_output=${subject}_${session}_acq-MRP_run-001_${res}_desc-cropped_T2w_seg.nii.gz
resized_T2_cropped=${subject}_${session}_acq-MRP_run-001_${res}_desc-cropped_T2w.nii.gz
resized_T2=${subject}_${session}_acq-MRP_run-001_${res}_T2w.nii.gz
seg_native=${subject}_${session}_acq-MRP_run-001_${res}_desc-native-headerchange_T2w_seg.nii.gz

# if false; then

python ../register-BCP-BOPS/ants_rigid_register.py \
    --fixed ${derivatives_dir}/${resized_T2} \
    --moving ${derivatives_dir}/${resized_T2_cropped} \
    --out_warped ${derivatives_dir}/${subject}_${session}_acq-MRP_run-001_${res}_desc-cropped-resize-space_T2w.nii.gz \
...
python estimate-dice.py \
  ...
  ${derivatives_dir}/${subject}_${session}_acq-MPR_run-001_T2w_dice-scores-BOPS-labels.csv
...
SS_QC_merged_labels=${derivatives_dir}/${subject}_${session}_acq-MPR_run-001_desc-SS-QC-merged-labels_T2w_labels.nii.gz
...
python estimate-dice.py ${groundtruth_QC_merged_labels} ${SS_QC_merged_labels} LUT.qc-labels.txt ${derivatives_dir}/${subject}_${session}_acq-MPR_run-001_T2w_dice-scores-qc-labels.csv
